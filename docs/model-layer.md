...
| `warm_start_checkpoint` | `null` (fresh init) | Opt-in transfer-learning hook: an artifact-store-relative path to another `TrainingRun`'s saved checkpoint (e.g. `models/{model_id}/training_{run_id}/best.pt`, the same convention `TrainingCheckpoint.artifact_path` already uses). If set, that checkpoint's weights are loaded into this run's freshly-built model immediately after construction, *before* the optimizer is created — so the optimizer always starts clean, letting a fine-tuning run be compared against a from-scratch run at otherwise identical hyperparams (same `max_steps`/`val_every_steps`/`disable_lr_scheduler`/seed/batch_size/etc — see "Comparing training runs" below). Only `load_state_dict`'s own shape check applies: the caller is responsible for giving the warm-started run the same `feature_cols`/`tgt_feature_cols`/`obs_len`/`pred_len`/architecture config as the run that produced the checkpoint. Used for pretrain-on-synthetic-data → fine-tune-on-real-data experiments (e.g. DDM → USDJPY). Also supported under `execution_target="colab"` — see `docs/colab-workflow.md`'s note on `warm_start_checkpoint` for the Drive-transport mechanism a Colab run needs to receive the checkpoint. |
| `token_level` | `null` (src == tgt, current/default behaviour) | Opt-in — decouples how the model's *input* history (src) is represented from `normalize`, which continues to control the *target* (tgt) exclusively. `"diff"`: src = raw first differences (continuous). `"quantize_diff"`: src = raw first differences discretized into `n_bins` integer token ids via quantile bin edges fit on the training split only. `"cluster"`: src = pattern/shape tokens — k-means (`n_clusters`) over sliding, per-window z-scored shapes of `cluster_window` consecutive differences, fit on the training split only (groups short movement patterns like "uptrend"/"range"/"sharp drop" into one token per shape, rather than one token per raw value). `"digits"`: src = recursive/base-10 digit tokens — each step's difference scaled to an integer (scale fit from the training split's 99.9th percentile) and represented as 1 sign token + `n_digits` digit tokens (0-9), so the model sees numeric *structure* (place value) instead of one opaque symbol per level. The only `token_level` that expands one time step into multiple token *positions* (src windows are `[obs_len, 1+n_digits]`, flattened to a length-`obs_len*(1+n_digits)` sequence by the embedding path — see `LSTMModel`/`Seq2SeqTransformer`). `"sax"`: src = classic SAX symbols — PAA (non-overlapping segment means, factor `sax_paa_size`) then the same quantile discretization as `quantize_diff`, into `n_bins` symbols; each src array position then represents `sax_paa_size` underlying time steps, and `tgt` is realigned to the last underlying step of each PAA segment. Every value regresses the same continuous target with the same MSE loss regardless of `token_level`, so results across different input representations are directly comparable via `val_loss` alone — see `OHLCWindowDataset`'s docstring in `backend/model/trainers/dataset.py` for the full rationale and the index-alignment contract between src and tgt. Only supported with a single `feature_cols` entry. Only implemented for `lstm` and `seq2seq_transformer` (the two architectures with an explicit embeddable input seam); other architectures ignore it. |
| `n_bins` | `7` | Only used when `token_level="quantize_diff"`. Vocabulary size for the discretized delta symbols. Bin edges are quantile-based (equal-frequency), not equal-width — a differenced periodic signal's histogram is typically U-shaped (density concentrated at the extremes, not at zero: e.g. for a pure sine wave the derivative is arcsine-distributed), so equal-width bins would produce wildly unbalanced token frequencies. |
| `cluster_window` | `20` | Only used when `token_level="cluster"`. Length (in differences) of the shape window clustered into a pattern token. Each shape is z-scored individually before clustering (removes that window's own scale, the standard normalization for shape/shapelet clustering) so windows group by movement *pattern*, not by local volatility. |
| `n_clusters` | `20` | Only used when `token_level="cluster"`. Vocabulary size (k in k-means). On a periodic signal this should recover something close to a phase partition of the cycle — the same shape recurs every `period` steps, so the cluster id sequence repeats at that lag far more than the `1/n_clusters` chance rate (a useful sanity check that clustering found real structure; see `tests/unit/test_ohlc_window_dataset.py::TestClusterTokenLevel`). |
| `n_digits` | `3` | Only used when `token_level="digits"`. Number of base-10 digit token positions per time step (plus 1 fixed sign-token position). Vocabulary is always 12 (digits 0-9 + 2 sign tokens), regardless of `n_digits` — increasing it adds sequence length (more token positions per step) and decimal precision, not vocabulary size. |
| `sax_paa_size` | `5` | Only used when `token_level="sax"`. PAA segment length — the number of consecutive differences averaged into one value before quantile discretization (classic SAX, but with empirical quantile breakpoints rather than Gaussian ones — see `n_bins`'s note on why). Each src array position then represents `sax_paa_size` underlying time steps, not one; `tgt` is realigned to the last underlying step of each segment (`OHLCWindowDataset`'s `token_level="sax"` docstring has the exact index arithmetic). |
| `embedding_dim` | `null` (defaults to `hidden_dim`) | Only used when `token_level` produces integer tokens (`quantize_diff`, `cluster`, `digits`, `sax`), on `lstm`. Size of the `nn.Embedding` lookup table's output vector. Ignored for `seq2seq_transformer`, which always embeds directly to `d_model` — every downstream layer (positional encoding, attention) fixes on `d_model` as the one working dimension, so a separately-sized embedding would just need an extra projection back to it. |
...
`"quantize_diff"`, `"cluster"`, `"digits"`, or `"sax"` — `compute_token_characteristics()` in
