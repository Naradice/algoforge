...
| `token_level` | `null` (src == tgt, current/default behaviour) | Opt-in — decouples how the model's *input* history (src) is represented from `normalize`, which continues to control the *target* (tgt) exclusively. `"diff"`: src = raw first differences (continuous). `"quantize_diff"`: src = raw first differences discretized into `n_bins` integer token ids via quantile bin edges fit on the training split only. Every value regresses the same continuous target with the same MSE loss regardless of `token_level`, so results across different input representations are directly comparable via `val_loss` alone — see `OHLCWindowDataset`'s docstring in `backend/model/trainers/dataset.py` for the full rationale and the index-alignment contract between src and tgt. Only supported with a single `feature_cols` entry. Only implemented for `lstm` and `seq2seq_transformer` (the two architectures with an explicit embeddable input seam); other architectures ignore it. |
